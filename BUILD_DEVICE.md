...
## Установка через M5Launcher

Сборка выпускает два образа:

| файл | что внутри | как ставить |
|---|---|---|
| `kodzimim-app.bin` | только приложение | **M5Launcher** |
| `kodzimim-full.bin` | бутлоадер + таблица разделов + приложение | `esptool` по USB на адрес `0x0`, затирает весь флеш вместе с лаунчером |

### Файл на карту клади через картридер

Проверено на практике: заливка `.bin` в лаунчер **через Web UI даёт битый файл**, и
установка падает с `flash read failed`. Тот же самый файл, скопированный на
microSD напрямую с ноутбука через картридер, ставится без единой ошибки.

Почему Web UI портит файл — не выяснено. Пока просто не пользуйся им для
установки прошивок.

Если увидишь `flash read failed`, первым делом подозревай **способ доставки
файла**, а не сам файл. Мы потеряли на этом несколько часов, разбирая заголовки
образов, хотя достаточно было переложить `.bin` картридером.

### Постоянная ссылка на свежую сборку
